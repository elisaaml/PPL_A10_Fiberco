        ...
        <div class="products-row">
            ...
            <div class="product-cell">
                <form action="{{ route('team.destroy', $item->id) }}" method="POST" style="display: inline-block;">
                    <a href="{{ route('team.edit', $item->id) }}" class="btn btn-sm btn-icon"><ion-icon name="create-outline"></ion-icon></a>
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-sm btn-icon"><ion-icon name="trash-outline"></ion-icon></button>
                </form>
            </div>
        </div>
    @empty
        <div class="products-row">
            <div class="product-cell" colspan="5">There are no data.</div>
        </div>
    @endforelse
